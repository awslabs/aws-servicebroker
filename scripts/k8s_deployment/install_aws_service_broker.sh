...
EOF
    ...
    c=0
    while [ "$(kubectl get pods | grep -c 1/1)" != "2" ] ; do if [ $c -gt 30 ]; then echo "failed waiting for AWS Service Broker Pod to com up..."; exit 1 ; fi; sleep 2 ; c=$((c+1)) ; done
